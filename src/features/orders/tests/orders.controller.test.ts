...
vi.mock('@/features/orders/api/orders.service.js', () => ({
  ordersService: {
    findById: vi.fn(),
    restore: vi.fn(),
    findAll: vi.fn(),
...
vi.mock('@/features/orders/api/orders.workflow.js', () => ({
  isValidTransition: vi.fn(() => true),
}));

import { ordersController } from '@/features/orders/api/orders.controller.js';
import { ordersService } from '@/features/orders/api/orders.service.js';
