...
export const seedService = {
  async seedDatabase() {
    ...
    const categoryMap = new Map(CATEGORY_SEEDS.map((c) => [c.slug, c.id]));
    ...
    for (const req of sampleRequests) {
      await prisma.customCakeRequest.upsert({
        where: { id: req.id },
        update: req as any,
        create: req as any,
      });
    ...
    for (const item of GALLERY_ITEMS) {
      ...
      await prisma.cakeGalleryItem.upsert({
        where: { id: item.id },
    ...
    return {
      ...
      galleryItems: GALLERY_ITEMS.length,
    };
  },
};
